...
### MODIFIED BY TIM. BEGIN---
# ADD
event_table = Table(event_df, {"obj_names": ots})
ocel = OCEL(event_table)
...
F = (
    ...
    + [
        (
            feature_extraction.EVENT_AGG_PREVIOUS_CHAR_VALUES,
...
if True:
    ...
    for feat in feat_to_s.keys():
        ...
        data_df[name] = [
            feat_to_s[feat][0][0][i] for i in range(0, len(feat_to_s[feat][0][0]))
        ]
    plt.clf()
    ...
    ax2 = plt.twinx()
    sns.lineplot(
        data=data_df["Average Remaining Time in s"],
        ax=ax2,
        color="#DD8452",
        linewidth=2,
    )
    ...
    plot_.set_xlabel("Date", fontsize=14)
    plt.tight_layout()
...
if True:
    ...
    train_table = tabular.construct_table(
        ...
        exclude_events=events_to_remove,
    )
    ...
    mapping_names = {
        feature_extraction.EVENT_PRECEDING_ACTIVITES: "Prec. activities:",
...
if True:
    ...
    k = 4
    features = [
        feat for feat in F if feat != (feature_extraction.EVENT_REMAINING_TIME, ())
    ]
    ...
    best_weights_callback = tf.keras.callbacks.ModelCheckpoint(
        "lstm_checkpoint.h5", monitor="val_loss", save_best_only=True, verbose=1
    )
    ...
    explainer = shap.KernelExplainer(
        f, X100.reshape(X100.shape[0], X100.shape[1] * X100.shape[2])
    )  # , feature_names=[str(f) for f in features])
    shap_values = explainer.shap_values(
        x_train[:number_shap].reshape(
            x_train[:number_shap].shape[0], x_train[3].shape[0] * x_train[3].shape[1]
        ),
        nsamples=100,
    )
    ...
    for i in range(0, len(shap_values)):
        ...
        ax = sns.heatmap(
            np.transpose(new_shap),
            xticklabels=["-3 events", "-2 events", "-1 event", "current event"],
