...
def generate_graph_dataset(feature_graph_list, indices, ocel: OCEL):
    ...
    for gid in tqdm(idx):
        ...
        dgl_graph = dgl.graph(
            data=(
                [node_id_map[e.source] for e in graph.edges],
        ...
        if len(sorted_node_indices) != 0:
            for i in range(k - 1, len(sorted_node_indices)):
                subgraph = dgl.node_subgraph(
                    dgl_graph, nodes=range(i - (k - 1), i + 1)
                )  # include last event
    ...
    def __getitem__(self, idx):
        ...
        if self.make_bidirected:
            with tf.device("CPU:0"):
                graph_batch = [
                    dgl.to_bidirected(g, copy_ndata=True) for g in graph_batch
                ]
        if self.on_gpu:
